    ...
    <PrivateRoute>
      <Layout>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <Card title="Total de Clientes" value={dashboard?.totalClientes || 0} icon={<Users className="w-6 h-6 text-white" />} />
          <Card title="Total de Fichas" value={dashboard?.totalFichas || 0} icon={<ClipboardList className="w-6 h-6 text-white" />} />
          <Card title="Fichas este mês" value={dashboard?.fichasPorMes?.[0]?.total || 0} icon={<CalendarDays className="w-6 h-6 text-white" />} />
        </div>
      </Layout>
    </PrivateRoute>
  );
